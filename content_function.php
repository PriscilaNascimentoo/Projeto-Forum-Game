<?php
	...
	function dispsubcategories($parent_id) {
		...
		while ($row = mysqli_fetch_assoc($select)) {
			echo "<tr><td class='category_title'><a href='/gamerstation/topics/".$row['cat_id']."/".$row['subcat_id']."'>
	...
	function disptopics($cid, $scid) {
		...
		if (mysqli_num_rows($select) != 0) {
			...
			echo "<tr><th>Título</th><th>Postado por</th><th>Data</th><th>Visualizações</th><th>Comentários</th></tr>";
			while ($row = mysqli_fetch_assoc($select)) {
				echo "<tr><td><a href='/gamerstation/readtopic/".$cid."/".$scid."/".$row['topic_id']."'>
					 ".$row['title']."</a></td><td>".$row['author']."</td><td>".$row['date_posted']."</td><td>".$row['views']."</td>
					 <td>".$row['replies']."</td></tr>";
			}
			echo "</table>";
		} else {
			echo "<p>Essa categoria não tem nenhum tópico ainda.  <a href='/gamerstation/newtopic/".$cid."/".$scid."'>
				 Que tal adicionar o primeiro?</a></p>";
		}
	}
	...
	function replylink($cid, $scid, $tid) {
		echo "<p><a href='/gamerstation/replyto/".$cid."/".$scid."/".$tid."'>Responder a este tópico</a></p>";
	}
	
	function replytopost($cid, $scid, $tid) {
		echo "<div class='content'><form action='/gamerstation/addreply/".$cid."/".$scid."/".$tid."' method='POST'>
			  <p>Comment: </p>
